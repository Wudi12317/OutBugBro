...
func _process_charge_input() -> void:
	if _shooting.aim_mode != ShootingComponent.AimMode.MOUSE:
		return
...
func _show_mode_tip() -> void:
	var text := "自动瞄准" if _shooting.aim_mode == ShootingComponent.AimMode.AUTO else "手动模式 (蓄力射击!)"
