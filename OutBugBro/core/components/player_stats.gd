## 玩家属性组件 — 升级加成 + 元进度加成 + 效果加成计算
## 防御公式：每 8 点防御，受到伤害 -1（下限 0）
...
## 获取基础值（含升级加成 + 元进度加成，不含效果）
func _get_base(stat: String) -> float:
	var base: float = _cfg_base(stat) + _cfg_per_level(stat) * upgrade_levels[stat]
	# 元进度加成
	match stat:
		"max_hp":   base += MetaProgression.get_hp_bonus()
		"damage":   base += MetaProgression.get_atk_bonus()
		"defense":  base += MetaProgression.get_def_bonus()
		"move_speed": base += MetaProgression.get_spd_bonus()
	return base
...
func get_damage() -> int:
	...
	var meta_mult := MetaProgression.get_dmg_mult_bonus()
	return maxi(1, int((_get_base("damage") + flat) * (1.0 + mult + meta_mult)))

func get_crit_rate() -> float:
	return clampf(_get_base("crit_rate") + _get_sum("crit_rate_change") + MetaProgression.get_crit_bonus(), 0.0, 1.0)
...
func get_hp_regen() -> float:
	return MetaProgression.get_hp_regen()
