...
func _cast_summon() -> void:
	# 受 max_enemies 限制，避免场上敌人过多
	var enemies := get_tree().get_nodes_in_group("enemies")
	var alive_count := 0
	for e in enemies:
		if is_instance_valid(e) and not e.has_meta("dead"):
			alive_count += 1
	var max_allowed := 30
	var count := mini(3 + (randi() % 2), max_allowed - alive_count)
	if count <= 0:
		return
	# 优先使用 .tscn 场景
	var fast_scene := load("res://scenes/enemies/fast_enemy.tscn") as PackedScene
	var ranged_scene := load("res://scenes/enemies/ranged_enemy.tscn") as PackedScene
	for i in range(count):
		var scene: PackedScene = fast_scene if randf() < 0.4 else ranged_scene
		var enemy: CharacterBody2D = scene.instantiate() if scene else (FastEnemy.new() if randf() < 0.4 else RangedEnemy.new())
