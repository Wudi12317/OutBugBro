...
@onready var _challenge_btn: Button = %ChallengeBtn
...
func _ready() -> void:
	...
	# 按钮动效
	for btn: Button in [_start_btn, _continue_btn, _intro_btn, _about_btn, _quit_btn, _challenge_btn]:
	...
	# 挑战模式特殊样式
	_apply_challenge_btn_style(_challenge_btn)
	# 弹窗毛玻璃暗色样式
...
## 挑战模式按钮样式（金色边框）
func _apply_challenge_btn_style(btn: Button) -> void:
	var style := StyleBoxFlat.new()
...
func _update_challenge_score() -> void:
	...
	if json.parse(f.get_as_text()) == OK and json.data is Dictionary:
		var best: float = json.data.get("best_score", 0.0)
		if best > 0.0:
			_challenge_btn.text = "⚔  挑战模式 (最高: %.1f)" % best
	f.close()
